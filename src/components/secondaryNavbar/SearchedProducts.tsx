...
const SearchedProducts = () => {
	...
	const searchProductData: ProductFormValues = state?.products;
								...
								<div>
									...
									>
										<Text className="text-lg mt-0" color={priceTextColor}>
											Rs.{' '}
											{Number(
												searchProductData.discountedPrice
											).toLocaleString()}
										</Text>
										<Text
										...
										>
											Rs.{' '}
											{Number(searchProductData.originalPrice).toLocaleString()}
										</Text>
									</div>
								</div>
								<Flex
									onClick={() => {
										handleToggleWishlist(searchProductData._id);
									}}
									className={`heart-button flex flex-col-reverse mb-1 mr-4 group cursor-pointer ${
										wishlistItems.includes(searchProductData._id)
											? 'is-active'
											: ''
									}`}
								>
